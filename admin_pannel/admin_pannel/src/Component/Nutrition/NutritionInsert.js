...
function NutritionLogInsert() {
  ...
  const [mycalories, setCalories] = useState();
  ...
  const handleFoodChange = (index, e) => {
    ...
    if (field === 'macros') {
      newFoods[index].macros[key] = numericValue;
    } else {
      newFoods[index][field] = numericValue;
    }

    // Recalculate calories if quantity is updated
    if (field === 'quantity') {
      console.log('aaa')
      // newFoods[index].calories =numericValue;
      newFoods[index].calories =(mycalories/100)*numericValue;
    //  (item.calories / item.serving_size_g) * newServingSize
    }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.data.items.length === 0) {
        ...
        const defaultQuantity = servingSize || item.serving_size_g;
        ...
        setFormData(prevState => ({
          ...
          foods: [{
            ...
            calories: calculateCalories(item, defaultQuantity),
            macros: {
  ...
  const calculateCalories2 = (food, newQuantity) => {
    console.log('Calculating calories with:', food, newQuantity); 
    if (food.quantity <= 0 || newQuantity <= 0) return 0;
    return (food.calories / food.quantity) * newQuantity;
  };
        ...
        {nutritionData && nutritionData.items && (
          <div style={styles.resultContainer}>
            {nutritionData.items.map((item, index) => (
              <div key={index} style={styles.resultItem} className="row text-start">
                <h1 className="text-center" style={styles.resultHeader}>{item.name}</h1>
                <p className="col-12 text-center">Calories: {calculateCalories(item, servingSize || item.serving_size_g)}</p>
                <p className="col-6">Serving Size: {item.serving_size_g}g</p>
                <p className="col-6">Fat (Total): {item.fat_total_g}g</p>
                <p className="col-6">Fat (Saturated): {item.fat_saturated_g}g</p>
                <p className="col-6">Protein: {item.protein_g}g</p>
                <p className="col-6">Sodium: {item.sodium_mg}mg</p>
                <p className="col-6">Potassium: {item.potassium_mg}mg</p>
                <p className="col-6">Cholesterol: {item.cholesterol_mg}mg</p>
                <p className="col-6">Carbohydrates (Total): {item.carbohydrates_total_g}g</p>
                <p className="col-6">Fiber: {item.fiber_g}g</p>
                <p className="col-6">Sugar: {item.sugar_g}g</p>
                <div>
